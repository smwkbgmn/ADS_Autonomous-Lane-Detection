...
class LKASVisualizer:
    ...
    def draw_vehicle_position(self,
                            ...
                            departure_status: LaneDepartureStatus) -> np.ndarray:
        """
    ...
    def draw_hud(self,
                ...
                vehicle_telemetry: Optional[Dict] = None) -> np.ndarray:
        """
        ...
        cv2.rectangle(overlay, (0, 0), (output.shape[1], hud_height),
                     self.COLOR_BLACK, -1)
        output = cv2.addWeighted(output, 0.7, overlay, 0.3, 0)
        ...
        if vehicle_telemetry:
            ...
            cv2.putText(output, f"Speed: {speed:.1f} km/h",
    ...
    def _draw_steering_indicator(self, image: np.ndarray, steering_value: float):
        ...
        color = self.COLOR_GREEN if abs(steering_value) < 0.3 else \
                self.COLOR_YELLOW if abs(steering_value) < 0.6 else \
                self.COLOR_RED
    ...
    def create_alert_overlay(self,
        ...
        if departure_status in [LaneDepartureStatus.LEFT_DEPARTURE,
        ...
        elif departure_status in [LaneDepartureStatus.LEFT_DRIFT,
    ...
    def create_combined_view(self,
                           ...
                           metrics: Dict,
                           steering_value: Optional[float] = None) -> np.ndarray:
        """
        ...
        combined = self.draw_hud(combined, metrics, show_steering=True,
                                steering_value=steering_value)

        # Add labels
        cv2.putText(combined, "ORIGINAL", (10, height - 10),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, self.COLOR_WHITE, 2)
        cv2.putText(combined, "LANE DETECTION", (width + 10, height - 10),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, self.COLOR_WHITE, 2)
...
if __name__ == "__main__":
    ...
    output = visualizer.draw_vehicle_position(output, 400,
                                             metrics['lane_center_x'],
                                             metrics['departure_status'])
    output = visualizer.draw_hud(output, metrics, show_steering=True,
                                steering_value=-0.3)
